...
void WsManager::onConnect() {                 // WStype_CONNECTED 回调
    if (_clients < 255) ++_clients;
    _lastState = 0;                           // 让新页面立刻收到首帧
}

void WsManager::taskLoop() {
    srv->loop();
...
state 帧形状（字段按实际模块增删，语义见 2.2 第 3 条）：

```json
{ "type":"state", "ts":12345678,
  "gpio":[{"pin":0,"state":1,"dir":1}],
  "led":{"pin":0,"mode":4,"mode_str":"cycle","on":1,"r":0,"g":255,"b":0,"ready":1},
  "pwm":{"active":1,"pin":0,"period":1000,"duty":25,"freq":1000,"resolution":12},
  "adc":[{"ch":0,"raw":2048,"voltage":3.300}],
  "adc_src":"<source>", "adc_ready":1 }
```
...
否则动态 `createElement` 后赋 `b.id = 'x'` 不会进 `getElementById` 索引，
拿到的是新的空 El，报 `xxx.onclick is not a function`。

断言覆盖建议（每个控件至少一条）：
- GPIO：给 (state,dir) → 文本同时含 HIGH/LOW 与 IN/OUT。
- LED：给 (mode,mode_str,on,r,g,b) → 文本含模式名与 `ON rgb(...)`，且选中按钮只加中性 class 不带颜色。
- PWM：给出 period/duty → 文本含 pin、周期、占空比、换算后的频率，且输入框被回填。
- ADC：给出 raw/voltage → 两者都出现在文本里，标题含数据源名。
...
- **Git Bash 下 node 读不到 `/tmp/x.js`**（`/tmp` 映射到某个盘符，与 node 的 Windows 路径解析不一致）
  → 脚本一律落到工程内 `tools/`，用相对路径跑。
- 断言脚本输出用**英文**，避免 Windows GBK 控制台乱码。
- 这两个坑只在"本地跑 Node/Python 验证脚本"时出现；烧录/构建脚本的坑见构建类 skill。
